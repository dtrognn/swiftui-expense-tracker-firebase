//
//  TextEditorConstant.swift
//  SpendSmart
//
//  Created by dtrognn on 08/04/2024.
//

import Foundation

enum TextEditorConst {
    static let TEXTEDITOR_DEFAULT_LINE_COUNT: Int = 1
    static let TEXTEDITOR_MAX_LINE_COUNT: Int = 5
    static let TEXTEDITOR_INSET_HORIZONTAL: CGFloat = 10
    static let TEXTEDITOR_WIDTH_HORIZONTAL_BUFFER: CGFloat = 17
    static let TEXTEDITOR_INSET_BOTTOM: CGFloat = 0
    static let TEXTEDITOR_STROKE_CORNER_RADIUS: CGFloat = AppStyle.layout.standardCornerRadius
    static let TEXTEDITOR_FRAME_HEIGHT_FREESPACE: CGFloat = 20
}
